...
const CartPage = () => {
  ...
  const { cart, removeFromCart, updateQuantity, updateMerchandiseQuantity, clearCart, getCartSubtotal } = useEventStore();
  ...
  const handleQuantityChange = (index: number, type: 'more' | 'less', maxQty?: number) => {
    updateQuantity(index, type, maxQty);
  };
  
  const handleMerchandiseQuantityChange = (cartIndex: number, merchIndex: number, type: 'more' | 'less') => {
  ...
  const handleApplyCoupon = () => {
    ...
    setTimeout(() => {
      ...
      //   .then(success => toast.success('Coupon applied!'))
            ...
            <Card className="bg-gray-900 border-gray-800">
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-gray-400 mb-6 text-center max-w-md">
                  Looks like you haven&apos;t added any events to your cart yet. Browse our events to find something you&apos;ll love!
                </p>
                <Button 
              ...
              <div className="lg:col-span-2">
                <Card className="bg-gray-900 border-gray-800 mb-6">
                  <CardHeader className="border-b border-gray-800">
                    <CardTitle className="text-white">Cart Items ({cart.length})</CardTitle>
                  </CardHeader>
                          ...
                          <div className="flex flex-col sm:flex-row gap-4">
                            ...
                            <div className="flex-1">
                              <div className="flex justify-between">
                                <div>
                                  <h3 className="text-white font-medium">{item.event.name}</h3>
                                  <p className="text-gray-400 text-sm">{item.ticketName}</p>
                                  <div className="flex gap-2 items-center mt-1">
                                    <span className="text-purple-400 font-medium">
                                      {currencyMapping[item.country] || '$'}{item.salePrice.toFixed(2)}
                                    </span>
                                  </div>
                                </div>
                                <Button 
                              ...
                              <div className="flex items-center mt-3">
                                <span className="text-sm text-gray-400 mr-2">Quantity:</span>
                                <div className="flex items-center border border-gray-700 rounded-md">
                                  <Button 
                                    variant="ghost" 
                                    size="icon"
                                    disabled={item.qty <= 1}
                                    onClick={() => handleQuantityChange(index, 'less')}
                                    className="h-8 w-8 text-gray-400 hover:text-white"
                                  >
                                    <Minus className="h-3 w-3" />
                                  </Button>
                                  <span className="w-10 text-center text-white">{item.qty}</span>
                                  <Button 
                                    variant="ghost" 
                                    size="icon"
                                    disabled={item.qty >= item.event.quantity}
                                    onClick={() => handleQuantityChange(index, 'more', item.event.quantity)}
                                    className="h-8 w-8 text-gray-400 hover:text-white"
                                  >
                            ...
                            <div className="mt-4 ml-0 sm:ml-40">
                              ...
                              <div className="space-y-2">
                                {item.merchandise.map((merch, merchIndex) => (
                                  ...
                                  >
                                    <span className="text-white text-sm">{merch.itemName}</span>
                                    <div className="flex items-center">
                                      <div className="flex items-center border border-gray-700 rounded-md mr-4">
                                        <Button 
                                          ...
                                          onClick={() => handleMerchandiseQuantityChange(index, merchIndex, 'less')}
                                          className="h-7 w-7 text-gray-400 hover:text-white"
                                        >
                                          <Minus className="h-3 w-3" />
                                        </Button>
                                        <span className="w-8 text-center text-white text-sm">{merch.qty}</span>
                                        <Button 
                                          variant="ghost" 
                                          size="icon"
                                          onClick={() => handleMerchandiseQuantityChange(index, merchIndex, 'more')}
                                          className="h-7 w-7 text-gray-400 hover:text-white"
                                        >
                                          <Plus className="h-3 w-3" />
                                        </Button>
                                      ...
                                      <span className="text-purple-400 text-sm font-medium w-20 text-right">
                                        {currencyMapping[item.country] || '$'}{(merch.price * merch.qty).toFixed(2)}
                                      </span>
                                    </div>
                                  </div>
                                ))}
